    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if p == "/api/backup":
                ...
                # ── Etapa 2: copiar para o Drive com verificação ──────────────
                ...
                for attempt in range(3):
                    try:
                        ...
                        time.sleep(0.5)
                        if os.path.exists(outpath) and os.path.getsize(outpath) == tmp_size:
                            copy_ok = True
                            break
                    except Exception:
                        time.sleep(1)
                ...
                if not copy_ok:
                    self._json(500, {
                        "error": f"Exportado para /tmp/ ({tmp_size} bytes), "
                                 f"mas falhou ao copiar para o Drive após 3 tentativas. "
                                 f"Arquivo temporário salvo em: {tmp_path}"
                    })
                    return
