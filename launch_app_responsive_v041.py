    ...
    async function _doRestoreSession(sessionId) {
      try {
        ...
        if (r.ok) {
          ...
          _sessionsCache = { data: null, ts: 0 };
          toast("✅ Sessão " + sessionId + " restaurada! Recarregando terminal…", "ok");
          closeSessions();
          // 🐛 CORREÇÃO v0.5.1.8: recarrega o iframe do ttyd com a sessão restaurada,
          // mesmo padrão usado em doRestore() / toggleTheme() / confirmProvider().
          // O terminal precisa ser reiniciado para refletir o estado da sessão.
          reloadTerminalFrame();
        } else {
