...
## 8. Arquitetura Técnica (v0.3.0)

A partir da versão 0.3, o PesquisAI é distribuído como um pacote Python modular:

```
pesquisai/
├── config.py                # Settings pydantic 2 (frozen)
├── run.py                   # Orquestrador principal
├── progress.py              # Barra de progresso (Colab + terminal)
...
│   ├── server.py            # HTTP wrapper (12 rotas, seguro)
│   ├── templates/           # HTML estático
│   └── static/              # CSS + JS separados
└── utils/
    ├── security.py          # Validação de comandos, path traversal
    ├── opencode.py          # Localização/instalação opencode
    └── subprocess.py        # CommandRunner Protocol + FakeRunner
```

### 8.1 Princípios de Código

| Princípio | Aplicação |
|---|---|
| **Type hints** | ~95% das funções públicas tipadas |
| **Injeção de dependência** | `make_handler(runner)` para o HTTP server |
| **Testes** | `pytest` com 43 testes (todos passando) |
| **Logging** | `logging.getLogger("pesquisai.<modulo>")` por módulo |
| **Segurança** | Whitelist de comandos, validação de path, `0o600` em credenciais |
| **Separação** | HTML/JS/CSS em arquivos estáticos; cada rota HTTP em função própria |
...
# Validar importações
python -c "import pesquisai; print(pesquisai.__version__)"

# Executar o PesquisAI (no Colab)
python main.py
```

---

## 9. Exemplo de Resposta Estruturada
...
*PesquisAI · v0.3.0 · Registro SisPPG/UFV nº 10356285004 · Mantido em conformidade com os princípios de integridade científica da CAPES e CNPq*
