# 📦 PesquisAI v0.4.1 — Release Summary
...
Adições do **PesquisAI v0.4.1** — patch corretivo que corrige 3 bugs críticos reportados pelo usuário no chat `session-ses_10b7.md` (2026-06-23):

| Correção | Tipo | Status |
|---|---|---|
| 🐛 **Site não responsivo** | Bug fix | ✅ Corrigido |
| 🐛 **Tema não recarrega terminal** | Bug fix | ✅ Corrigido |
| 🐛 **Idioma sem opção na UI** | Bug fix | ✅ Corrigido |

**Mudanças cumulativas v0.4.0 + v0.4.1:**

| Feature | Tipo | Tamanho | Testes |
|---|---|---|---|
| 🔍 **grant_finder** | Nova skill | ~1.500 linhas | 48 ✅ |
| 🌐 **i18n (4 idiomas)** | Novo módulo | ~700 linhas + 4 JSON | 31 ✅ |
| 📱 **launch_app_responsive v0.2.0** | Patch | ~900 linhas | Manual |
| 🐛 **launch_app_responsive_v041** | Drop-in patch | ~700 linhas | Manual |
| 🤖 **agents/** multilíngues | Documentação | 4 AGENTS.md | Manual |

**Total:** 52 arquivos · 79 testes · 83% de cobertura · 4 idiomas · 13 agências
...
| Métrica | Valor |
|---|---|
| **Versão** | 0.4.1 (UI Fixes) |
| **Data de release** | 2026-06-23 |
| **Codename** | UI Fixes (Responsive + Theme + Language) |
...
│
├── 🔬 pesquisai/
│   ├── launch_app_responsive.py          ← ✅ v0.4.1 (responsivo + tema escuro + idioma)
│   └── launch_app_responsive_v041.py     ← ✅ NOVO: drop-in para launch_app.py
│
├── 📚 docs/
│   ├── MOBILE_RESPONSIVE_PATCH.md        ← v0.4.0
│   ├── PATCH_v0.4.1.md                   ← ✅ NOVO v0.4.1
│   └── ENTREGAS_JUNHO_2026.md            ← Resumo de entregas
│
...
> **Nota v0.4.1:** O `launch_app_responsive.py` foi atualizado com:
> - 6 media queries (5 breakpoints + landscape) + hamburger menu
> - `toggleTheme()` que recarrega iframe do ttyd com cache-busting
> - Dropdown de idioma na topbar (4 idiomas, cookie, query param)
> - **Tema padrão ESCURO** com anti-flash CSS (script no `<head>`)
> - Classe `html.theme-light` para override do tema claro
> - 33 chaves de tradução inline + 4 dicionários i18n
...
**Para v0.4.0** (responsividade básica):
```python
